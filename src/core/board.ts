...
export function legalMoves(b: Board): Move[] {
  ...
  for (let idx = 0; idx < b.length; idx++) {
    ...
    if (p.o === 'H') {
      for (let c = p.c - 1; c >= 0 && (row[c] as number) === -1; c--)
        moves.push({ idx, nr: p.r, nc: c })
      const tail = p.c + p.len - 1
      for (let c = tail + 1; c < SIZE && (row[c] as number) === -1; c++)
        moves.push({ idx, nr: p.r, nc: p.c + (c - tail) })
    } else {
      for (let r = p.r - 1; r >= 0 && ((g[r] as number[])[p.c] as number) === -1; r--)
        moves.push({ idx, nr: r, nc: p.c })
      const tail = p.r + p.len - 1
      for (let r = tail + 1; r < SIZE && ((g[r] as number[])[p.c] as number) === -1; r++)
        moves.push({ idx, nr: p.r + (r - tail), nc: p.c })
    }
  }
  return moves
}
...
export function encode(b: Board): string {
  // unique per arrangement (fixed piece order)
  let s = ''
  for (const p of b) s += String.fromCharCode(p.r * SIZE + p.c)
  return s
}
export function decode(template: Board, s: string): Board {
  return template.map((p, i) => ({
    ...p,
    r: Math.floor(s.charCodeAt(i) / SIZE),
    c: s.charCodeAt(i) % SIZE,
  }))
}

...
it('round-trips state and recovers from corrupt storage', () => {
  const kv = memKV();
  save({ levelIndex: 5, muted: true, lang: 'eu' }, kv);
  expect(load(kv)).toEqual({ levelIndex: 5, muted: true, lang: 'eu' });
  kv.setItem('tutu.v1', '{not json');
  expect(load(kv).levelIndex).toBe(0); // falls back, doesn't throw
});
it('returns defaults when storage is empty', () => {
  expect(load(memKV())).toEqual({ levelIndex: 0, muted: false, lang: 'en' });
});
